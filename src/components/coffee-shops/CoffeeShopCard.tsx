...
const CoffeeShopCard: React.FC<CoffeeShopCardProps> = ({ coffeeShop, onClick }) => {
  const { name, shortDescription, type, rating, image } = coffeeShop;
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <h3 className="text-lg font-bold text-gray-800 mb-1 dark:text-white">{name}</h3>
        
        <div className="flex items-center text-amber-500 mb-2">
          {renderRating()}
          <span className="text-sm text-gray-600 ml-1 dark:text-gray-300">{rating.toFixed(1)}</span>
        </div>
        
        <p className="text-gray-600 text-sm mb-4 line-clamp-2 dark:text-gray-300">{shortDescription}</p>
